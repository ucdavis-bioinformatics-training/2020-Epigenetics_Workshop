...
myQuestions2 = [
  {
    question: "Try to add a new element to a factor. When the new element does not match any of the levels of the factor, what will happen?",
    answers: {
...
myQuestions4 = [
  {
    ...
    answers: {
      a: "209.2743 without a warning",
      b: "209.2743 with a warning",
      c: "210.5461 without a warning",
      d: "210.5461 with a warning"
    },
    correctAnswer: "b"
  },
  {
...
data1 <- as.data.frame(matrix(rnorm(49), ncol=7), stringsAsFactors=F)
dim(data1)

# calculate the sum for each row
lapply(1:dim(data1)[1], function(x){sum(data1[x,])})

# comparing the results to apply() results
apply(data1, MARGIN=1, sum)

# calculate log10 of the sum of each row
lapply(1:dim(data1)[1], function(x){log10(sum(data1[x,]))})
```
...
sapply(1:dim(data1)[1], function(x){log10(sum(data1[x,]))})
```

##### If the "simplify" parameter is turned off, sapply() will produced exactly the same results as lapply(), in the form of a list. By default, "simplify" is turned on.
```{r}
sapply(1:dim(data1)[1], function(x){log10(sum(data1[x,]))}, simplify=FALSE)
```
...
myQuestions5 = [
  ...
  {
    question: "Now, expand from the previous question to get only those rows whose standard deviation is greater than 20000. What is the row name?",
    answers: {
      a: "AT1G21310",
      b: "AT3G09260",
      c: "AT1G67090",
...
```{r}
boxplot(data1, xlab="Sample ID", ylab="Raw Counts")
```
