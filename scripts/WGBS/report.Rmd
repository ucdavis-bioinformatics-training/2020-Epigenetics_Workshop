...
#save(dm, file="DML.RData")
load("DML.RData")

#
dmls <- callDML(dmlTest, p.threshold=1.0)
write.table(dmls, file="Differential_methylation_CpG_EM24Mvs3M.txt", sep="\t", col.names=T, row.names=F, quote=F)
...
kable(dmls[1:500,], align = 'c', row.names=F) %>%
  ...
  add_header_above(header = c("Top 500 CpG sites" = 1:3, fill), font_size = "larger", bold = T, align = "l") %>%
...
write.table(dmrs, file="Differential_methylation_regions_EM24Mvs3M.txt", sep="\t", col.names=T, row.names=F, quote=F)
```
...
kable(dmrs, align = 'c', row.names=F) %>%
  add_header_above(header1, align = 'l') %>%
  add_header_above(header = c("DMRs" = 1, fill), font_size = "larger", bold = T, align = "l") %>%
  kable_styling(bootstrap_options = c("hover", "condensed", "responsive"), full_width = T) %>%
  scroll_box(height = "500px")
```
